...
#game-wrap{{height:100vh;display:none}}

/* ── SECTIONS ── */
.section{{min-height:100vh;scroll-snap-align:start;position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 24px}}

/* ── GRID / ANIMATED BG ── */
.grid-bg{{position:fixed;inset:0;z-index:0;pointer-events:none;
  background-image:linear-gradient(rgba(74,144,217,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(74,144,217,.055) 1px,transparent 1px);
...
.nav-link{{font-size:11px;letter-spacing:1px;text-transform:uppercase;color:#3a4d68;
  padding:6px 12px;border-radius:6px;cursor:pointer;transition:all .2s;border:none;background:none}}
.nav-link:hover,.nav-link.active{{color:#c8d8f0;background:rgba(74,144,217,.1)}}
.nav-enter{{font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;
  background:#4a90d9;color:#fff;border:none;border-radius:6px;padding:7px 16px;cursor:pointer;transition:all .2s}}
.nav-enter:hover{{background:#5aa0e9;transform:translateY(-1px)}}

/* ── HERO ── */
#hero{{padding-top:100px}}
.avail-pill{{display:inline-flex;align-items:center;gap:8px;
  background:rgba(62,207,142,.08);border:1px solid rgba(62,207,142,.25);
  border-radius:20px;padding:5px 14px;font-size:11px;letter-spacing:1.5px;
  color:#3ecf8e;text-transform:uppercase;margin-bottom:26px;
...
.hero-name{{font-size:clamp(42px,7vw,72px);font-weight:700;color:#eef2ff;
...
.hero-role{{font-size:clamp(13px,1.8vw,17px);color:#5a7096;
  letter-spacing:.5px;margin-bottom:14px;animation:fadeUp .8s .15s ease both}}
.hero-role strong{{color:#8aaccc;font-weight:500}}
.hero-summary{{font-size:14px;color:#4a6080;line-height:1.8;max-width:640px;
  margin:0 auto 30px;animation:fadeUp .8s .22s ease both}}
.hero-summary strong{{color:#7a9ab8;font-weight:500}}

.domain-row{{display:flex;flex-wrap:wrap;justify-content:center;gap:7px;
  margin-bottom:32px;animation:fadeUp .8s .28s ease both}}
.dtag{{font-size:10px;letter-spacing:.8px;text-transform:uppercase;
  padding:5px 12px;border-radius:5px;border:1px solid rgba(255,255,255,.09);
  color:#6a8aaa;background:rgba(255,255,255,.03);transition:all .2s;cursor:default}}
.dtag:hover{{border-color:rgba(74,144,217,.4);color:#b8d0e8;background:rgba(74,144,217,.07)}}

.stats-bar{{display:flex;border:1px solid rgba(255,255,255,.07);border-radius:10px;
  overflow:hidden;margin-bottom:30px;animation:fadeUp .8s .33s ease both}}
.stat-cell{{flex:1;padding:14px 18px;text-align:center;
  border-right:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.018)}}
.stat-cell:last-child{{border-right:none}}
.stat-n{{font-size:20px;font-weight:700;color:#eef2ff;margin-bottom:2px}}
...
.cta-row{{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;
  margin-bottom:26px;animation:fadeUp .8s .38s ease both}}
.btn-li{{display:inline-flex;align-items:center;gap:7px;background:rgba(10,102,194,.15);
  border:1px solid rgba(10,102,194,.35);border-radius:8px;color:#6aacdc;
  font-size:12px;font-weight:600;padding:11px 20px;cursor:pointer;
  letter-spacing:.4px;text-decoration:none;transition:all .2s}}
.btn-li:hover{{background:rgba(10,102,194,.25);color:#aad4f0;transform:translateY(-1px)}}
.btn-gh{{display:inline-flex;align-items:center;gap:7px;background:rgba(255,255,255,.05);
  border:1px solid rgba(255,255,255,.12);border-radius:8px;color:#8aaccc;
  font-size:12px;font-weight:600;padding:11px 20px;cursor:pointer;
  letter-spacing:.4px;text-decoration:none;transition:all .2s}}
.btn-gh:hover{{background:rgba(255,255,255,.1);color:#c8d8f0;transform:translateY(-1px)}}
.btn-enter{{display:inline-flex;align-items:center;gap:7px;
  background:#4a90d9;border:none;border-radius:8px;color:#fff;
  font-size:12px;font-weight:700;padding:11px 22px;cursor:pointer;
  letter-spacing:.4px;transition:all .2s}}
.btn-enter:hover{{background:#5aa0e9;transform:translateY(-1px)}}
.btn-resume{{display:inline-flex;align-items:center;gap:7px;
  background:rgba(62,207,142,.1);border:1px solid rgba(62,207,142,.25);
  border-radius:8px;color:#3ecf8e;font-size:12px;font-weight:600;
  padding:11px 20px;cursor:pointer;letter-spacing:.4px;text-decoration:none;transition:all .2s}}
.btn-resume:hover{{background:rgba(62,207,142,.18);transform:translateY(-1px)}}

.dist-chips{{display:flex;flex-wrap:wrap;justify-content:center;gap:6px;
  animation:fadeUp .8s .43s ease both}}
.dchip{{display:inline-flex;align-items:center;gap:6px;font-size:10px;
  letter-spacing:.5px;color:#2a3a52;padding:4px 10px;border-radius:4px;
  border:1px solid rgba(255,255,255,.05)}}
.dchip-pip{{width:6px;height:6px;border-radius:50%;flex-shrink:0}}
.scroll-cue{{margin-top:28px;font-size:9px;letter-spacing:2.5px;color:#1e2e42;
  text-transform:uppercase;animation:fadeUp .8s .5s ease both}}

@keyframes fadeUp{{from{{opacity:0;transform:translateY(14px)}}to{{opacity:1;transform:translateY(0)}}}}

/* ── SECTION SHARED ── */
.sec-eyebrow{{font-size:10px;letter-spacing:3px;color:#4a90d9;text-transform:uppercase;margin-bottom:10px}}
.sec-title{{font-size:clamp(26px,4vw,38px);font-weight:700;color:#eef2ff;
  letter-spacing:-1px;margin-bottom:8px}}
.sec-div{{width:32px;height:2px;background:#4a90d9;margin:0 0 32px}}

/* ── WHO AM I ── */
.who-grid{{display:grid;grid-template-columns:1fr 1fr;gap:20px;width:100%}}
@media(max-width:640px){{.who-grid{{grid-template-columns:1fr}}}}
.who-card{{background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.07);
  border-radius:12px;padding:22px}}
.who-card h3{{font-size:12px;letter-spacing:2px;color:#4a90d9;text-transform:uppercase;margin-bottom:14px}}
.who-item{{margin-bottom:14px}}
.who-item:last-child{{margin-bottom:0}}
.who-label{{font-size:10px;letter-spacing:1px;color:#2e4058;text-transform:uppercase;margin-bottom:3px}}
.who-value{{font-size:13px;color:#8aaccc;line-height:1.6}}
.who-value strong{{color:#c8d8f0;font-weight:500}}
.skill-chips{{display:flex;flex-wrap:wrap;gap:5px;margin-top:6px}}
.skill-chip{{font-size:10px;color:#5a7096;background:rgba(74,144,217,.08);
  border:1px solid rgba(74,144,217,.15);border-radius:4px;padding:3px 8px}}

/* ── EXPERIENCE ── */
.exp-item{{background:rgba(255,255,255,.022);border:1px solid rgba(255,255,255,.07);
  border-radius:12px;padding:22px;margin-bottom:14px;position:relative;
  border-left:3px solid #4a90d9}}
.exp-header{{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;gap:12px}}
.exp-role{{font-size:15px;font-weight:600;color:#eef2ff}}
.exp-date{{font-size:10px;letter-spacing:1px;color:#2e4058;text-transform:uppercase;
  white-space:nowrap;padding-top:3px}}
.exp-org{{font-size:12px;color:#4a90d9;margin-bottom:10px}}
.exp-bullets{{list-style:none;padding:0}}
.exp-bullets li{{font-size:12px;color:#5a7096;line-height:1.7;padding-left:14px;
  position:relative;margin-bottom:5px}}
.exp-bullets li::before{{content:'→';position:absolute;left:0;color:#4a90d9;font-size:10px;top:3px}}
.exp-tag{{display:inline-block;font-size:9px;letter-spacing:.5px;color:#4a90d9;
  ...
  border-radius:3px;padding:2px 7px;margin-right:5px;margin-bottom:4px;text-transform:uppercase}}

/* ── RESUME ── */
.resume-card{{background:rgba(255,255,255,.022);border:1px solid rgba(255,255,255,.07);
  border-radius:12px;padding:24px;width:100%;text-align:center}}
.resume-preview{{width:100%;height:500px;border:none;border-radius:8px;
  background:#0a1020;margin-bottom:16px}}
.resume-dl{{display:inline-flex;align-items:center;gap:8px;
  background:#4a90d9;border:none;border-radius:8px;color:#fff;
  font-size:12px;font-weight:700;padding:12px 28px;cursor:pointer;
  letter-spacing:.5px;text-decoration:none;transition:all .2s}}
.resume-dl:hover{{background:#5aa0e9;transform:translateY(-1px)}}

/* ── CONTACT ── */
.contact-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;width:100%}}
.contact-card{{background:rgba(255,255,255,.022);border:1px solid rgba(255,255,255,.07);
  border-radius:12px;padding:20px;text-align:center;transition:all .2s;text-decoration:none;display:block}}
.contact-card:hover{{background:rgba(74,144,217,.06);border-color:rgba(74,144,217,.25);transform:translateY(-2px)}}
.contact-icon{{font-size:28px;margin-bottom:10px}}
.contact-lbl{{font-size:9px;letter-spacing:2px;color:#2e4058;text-transform:uppercase;margin-bottom:5px}}
.contact-val{{font-size:13px;color:#8aaccc;font-weight:500}}
.explore-banner{{background:linear-gradient(135deg,rgba(74,144,217,.12),rgba(74,144,217,.04));
  border:1px solid rgba(74,144,217,.2);border-radius:12px;padding:28px;
  text-align:center;margin-top:20px;width:100%}}
.explore-banner h3{{font-size:18px;font-weight:600;color:#eef2ff;margin-bottom:8px}}
.explore-banner p{{font-size:13px;color:#4a6080;margin-bottom:18px;line-height:1.7}}
...
#hint-box{{position:fixed;bottom:16px;left:16px;display:none;background:rgba(4,8,15,.88);border:1px solid rgba(255,255,255,.06);border-radius:8px;padding:7px 13px;font-size:10px;color:#243040;z-index:50;line-height:2}}
...
.dp-btn{{background:rgba(4,8,15,.88);border:1px solid rgba(74,144,217,.2);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:16px;color:#3a5a7a;cursor:pointer;user-select:none;-webkit-user-select:none;touch-action:none;transition:all .1s}}
...
#back-btn{{position:fixed;top:14px;left:14px;z-index:201;display:none;background:rgba(4,8,15,.88);border:1px solid rgba(74,144,217,.2);border-radius:8px;color:#4a90d9;font-size:11px;font-weight:600;padding:8px 14px;cursor:pointer;letter-spacing:.5px;transition:all .2s}}
...
<nav id="nav">
  ...
  <div class="nav-links">
    <button class="nav-link active" onclick="scrollTo('hero')">Home</button>
    <button class="nav-link" onclick="scrollTo('who')">About</button>
    <button class="nav-link" onclick="scrollTo('exp')">Experience</button>
    <button class="nav-link" onclick="scrollTo('resume-sec')">Resume</button>
...
<section class="section" id="hero">
  <div class="section-inner" style="text-align:center">
    ...
    <div class="cta-row">
      ...
      <a class="btn-resume" href="data:application/pdf;base64,{RESUME_B64}" download="Mukund_Patel_Resume.pdf">
        ↓ Download Resume
      </a>
    ...
    <div class="who-grid">
      <div class="who-card">
        ...
        <div class="who-item">
          ...
          <div class="who-value"><strong>CVM University</strong></div>
          <div class="who-value">India &nbsp;·&nbsp; GPA: 3.58 / 4.0</div>
          <div class="who-value">Jun 2020 – Mar 2024</div>
        </div>
        <div class="who-item">
      ...
      <div class="who-card">
        ...
        <div class="who-item">
          ...
          <div class="skill-chips">
            ...
            <span class="skill-chip">R</span><span class="skill-chip">DAX</span>
            <span class="skill-chip">Power Query</span>
          </div>
        </div>
        <div class="who-item">
        ...
        <div class="who-item">
          ...
          <div class="skill-chips">
            ...
            <span class="skill-chip">PostgreSQL</span><span class="skill-chip">MySQL</span>
            <span class="skill-chip">SQLite</span><span class="skill-chip">GCP</span>
          </div>
        </div>
        <div class="who-item">
  ...
  <div class="section-inner">
    ...
    <div class="exp-item">
      ...
      <ul class="exp-bullets">
        <li>Automated data processing and validation for 8,500+ events using Python and Office Scripts, improving overall data quality.</li>
        ...
        <li>Built a Python computer vision pipeline detecting bus lane and bus stop violations from 900+ traffic cameras at 92% accuracy; integrated with Tableau dashboards for ticket issuance.</li>
        <li>Developed an ETL workflow processing driver trip logs to calculate pull-in/pull-out counts per depot at 30-minute intervals for fleet planning and maintenance optimization.</li>
      </ul>
    </div>
    ...
    <div class="exp-item" style="border-left-color:#9b6dff">
      ...
      <ul class="exp-bullets">
        <li>Standardized institution-wide student master data (8,000+ records) using SQL and spreadsheets for federal compliance and regulatory reporting.</li>
        <li>Created Tableau dashboards analyzing 5+ years of enrollment, financial aid, and academic outcome data; presented KPI insights to leadership stakeholders.</li>
      </ul>
    </div>
      ...
      <div class="exp-header">
        <div>
          ...
          <div style="margin-bottom:6px">
            ...
            <span class="exp-tag" style="color:#f5a623;background:rgba(245,166,35,.08);border-color:rgba(245,166,35,.2)">Community Analytics</span>
          </div>
        </div>
        <div class="exp-date">Apr 2025 – Aug 2025</div>
      </div>
      <ul class="exp-bullets">
        <li>Streamlined data cleaning, reducing processing time by 30%; structured raw data into categorized partner, focus-area, and population segments.</li>
        <li>Developed an interactive Python map and Power BI dashboards analyzing 150+ community investment activities to identify top-performing initiatives and key investment regions.</li>
      </ul>
    </div>
    ...
    <div class="exp-item" style="border-left-color:#3ecf8e">
      ...
      <ul class="exp-bullets">
        <li>Processed NJ weather data (1915–2024) and contributed to research demonstrating ARIMA and SARIMA models outperform deep learning in time and space efficiency for time-series forecasting.</li>
      </ul>
    </div>
  </div>
</section>
...
<section class="section" id="resume-sec">
  <div class="section-inner">
    ...
    <div class="resume-card">
      ...
      <br>
      <a class="resume-dl" href="data:application/pdf;base64,{RESUME_B64}" download="Mukund_Patel_Resume.pdf">
        ↓ &nbsp; Download Resume PDF
      </a>
    </div>
  </div>
...
<section class="section" id="contact">
  <div class="section-inner">
    ...
    <div class="explore-banner">
      <h3>Explore My Projects Interactively</h3>
      <p>Drive through a 3D city where each building represents a project — grouped by domain district. Pull up to any entrance and press Enter to view the full project details, live demo, and GitHub link.</p>
...
<div id="game-wrap">
  <canvas id="c"></canvas>
  <button id="back-btn" onclick="backToLanding()">← Back to Portfolio</button>
  <div id="hud">
    <div class="hud-box">
  ...
  <div id="hint-box"><span style="color:#8aaccc">W A S D</span> / Arrows — Drive &nbsp;·&nbsp; <span style="color:#8aaccc">Enter</span> — Open project</div>
  <div id="dpad">
...
(function(){{
  ...
  for(let i=0;i<80;i++) pts.push({{x:Math.random()*1400-700,y:Math.random()*800-400,vx:(Math.random()-.5)*.22,vy:(Math.random()-.5)*.22,r:Math.random()*1.6+.4,a:Math.random()}});
  function draw(){{
    ...
    for(let i=0;i<pts.length;i++){{
      ...
      cx.fillStyle=`rgba(74,144,217,${{p.a*.3}})`;cx.fill();
      for(let j=i+1;j<pts.length;j++){{
        ...
        if(d<110){{cx.beginPath();cx.moveTo(ox+p.x,oy+p.y);cx.lineTo(ox+pts[j].x,oy+pts[j].y);
          cx.strokeStyle=`rgba(74,144,217,${{(1-d/110)*.07}})`;cx.lineWidth=.5;cx.stroke();}}
      }}
    }}
...
landing.addEventListener('scroll',()=>{{
  const sections=['hero','who','exp','resume-sec','contact'];
  const links=document.querySelectorAll('.nav-link');
  sections.forEach((id,i)=>{{
    const el=document.getElementById(id);
    if(!el)return;
    const rect=el.getBoundingClientRect();
    if(rect.top<=80&&rect.bottom>=80){{links.forEach(l=>l.classList.remove('active'));links[i]&&links[i].classList.add('active');}}
  }});
}});
...
function startGame(){{
  ...
  if(!gameInitialised){{gameInitialised=true;initGame();}}
}}
...
const PROJECTS=[
  ...
  {{name:"Retail Intelligence Platform", icon:"🛒",district:"retail",  stat:"4 AI modules · Live app",    cat:"Streamlit Application",   stack:"Python · Groq LLM · Yelp Dataset",  desc:"AI-native customer intelligence platform converting public review data into operational insights. Four modules: Voice of Customer AI, Store Pulse Map, Test & Learn Autopilot (A/B), and Analyst Copilot (plain-English Q&A chatbot).",tags:["Groq LLM","NLP","Plotly","A/B Testing"],gh:"https://github.com/Mkp-7/retail-intelligence-platform",live:"https://retail-intelligence-platform-zvdrhjrf6hcmsuwgnvurd9.streamlit.app/?page=home"}},
  {{name:"Retail Customer Analytics",    icon:"🛍️",district:"retail",  stat:"25K products · -40% time",   cat:"ML & Python",             stack:"Python · Scikit-learn · K-Means",   desc:"Analyzed 25,000+ product records and 10,000+ transactions. K-Means clustering (5 segments) identified top 20% of customers contributing 60% of revenue. Automated Excel reporting dashboard tracking 10+ KPIs, reducing manual analysis time by 40%.",tags:["K-Means","Customer Segmentation","RFM","Pandas"],gh:"https://github.com/Mkp-7/Retail-Customer-Analytics-Reporting-System",live:""}},
  ...
  {{name:"MSU Collaboratory",            icon:"🎓",district:"research",stat:"3D network · MSU partners", cat:"Tableau Dashboard",       stack:"Tableau · 3D Network Graph",        desc:"3D network visualization of interdisciplinary activities and partnerships at Montclair State University across community partner organizations. Tableau dashboard surfacing Collaboratory data insights and cross-organizational collaboration topology.",tags:["Tableau","3D Network Graph","MSU","Community Partners"],gh:"https://mkp-7.github.io/Network-Graph/",live:"https://public.tableau.com/app/profile/mukund.patel7859/viz/Collaboratory_Data_Insights_Mukund-2/Dashboard1"}},
...
let carAngle=Math.PI,carSpeed=0,frame=0;
let mmCanvas,mmCtx,visitedSet=new Set();
let audioCtx=null,engineOsc=null,engineGain=null,lastNearEntry=null;
...
function projPos(p){{
  ...
  return{{x:-((cols-1)*BLK_X)/2+col*BLK_X,z:-((DIST_ORDER.length-1)*BLK_Z)/2+row*BLK_Z}};
}}

function buildCity(){{
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(CITY_H*2.5,CITY_H*2.5),new THREE.MeshLambertMaterial({{color:0x060a10}}));
...
function makeRoads(){{
  ...
  const ROWS=DIST_ORDER.length,startZ=-((ROWS-1)*BLK_Z)/2;
...
function makeHR(rz,rM,lM,sM){{
  ...
  const r=new THREE.Mesh(new THREE.PlaneGeometry(len,ROAD_W),rM);r.rotation.x=-Math.PI/2;r.position.set(0,0.05,rz);r.receiveShadow=true;scene.add(r);
  for(const oz of[-ROAD_W/2+0.3,ROAD_W/2-0.3]){{const k=new THREE.Mesh(new THREE.BoxGeometry(len,0.16,0.3),sM);k.position.set(0,0.08,rz+oz);scene.add(k);}}
...
function makeVR(rx,rM,lM,sM){{
  const len=CITY_H*2.2;
  const r=new THREE.Mesh(new THREE.PlaneGeometry(ROAD_W,len),rM);r.rotation.x=-Math.PI/2;r.position.set(rx,0.05,0);r.receiveShadow=true;scene.add(r);
  for(const ox of[-ROAD_W/2+0.3,ROAD_W/2-0.3]){{const k=new THREE.Mesh(new THREE.BoxGeometry(0.3,0.16,len),sM);k.position.set(rx+ox,0.08,0);scene.add(k);}}
...
function makeDistrictZones(){{
  ...
  DIST_ORDER.forEach((dk,row)=>{{
    const dist=DISTRICTS[dk],cols=ROW_COLS[dk],startX=-((cols-1)*BLK_X)/2;
    const zone=new THREE.Mesh(new THREE.PlaneGeometry(cols*BLK_X+3,BLK_Z-ROAD_W-1),new THREE.MeshLambertMaterial({{color:dist.color,transparent:true,opacity:0.038}}));
    zone.rotation.x=-Math.PI/2;zone.position.set(startX+(cols-1)*BLK_X/2,0.055,startZ+row*BLK_Z);scene.add(zone);
  }});
}}
function makeDistrictSigns(){{
  ...
  DIST_ORDER.forEach((dk,row)=>{{
    const dist=DISTRICTS[dk],cols=ROW_COLS[dk],startX=-((cols-1)*BLK_X)/2;
    const cx=startX+(cols-1)*BLK_X/2,rz=startZ+row*BLK_Z-BLK_Z/2+1.5;
...
function placeBuildings(){{PROJECTS.forEach((p,i)=>{{const{{x,z}}=projPos(p);buildings.push(makeBuilding(p,x,z,i));}});}}
function makeBuilding(p,bx,bz,idx){{
  const dist=DISTRICTS[p.district],hc=dist.color,style=idx%4,bH=9+(idx%7)*3.2,bW=7.5,bD=6.5;
  const g=new THREE.Group();g.position.set(bx,0,bz);
  const pave=new THREE.Mesh(new THREE.BoxGeometry(bW+3,0.2,bD+3),new THREE.MeshLambertMaterial({{color:0x0e1826}}));pave.position.y=0.1;pave.receiveShadow=true;g.add(pave);
  ...
  const roofBar=new THREE.Mesh(new THREE.BoxGeometry(bW,0.3,bD),new THREE.MeshLambertMaterial({{color:hc,emissive:hc,emissiveIntensity:0.55}}));roofBar.position.y=bH+0.38;g.add(roofBar);
  const rL=new THREE.PointLight(hc,0.65,22);rL.position.set(0,bH+2,0);g.add(rL);
  const pg=makeParticles(p.district,hc,bW,bH);g.add(pg);
  const eZ=bD/2+3.6;
  const lTex=makeRoadTex(p.name,dist.hex);
  const lMesh=new THREE.Mesh(new THREE.PlaneGeometry(9.5,2),new THREE.MeshBasicMaterial({{map:lTex,transparent:true,depthWrite:false}}));lMesh.rotation.x=-Math.PI/2;lMesh.position.set(0,0.11,eZ+3.8);g.add(lMesh);
...
function makeParticles(dk,color,bW,bH){{
  ...
  group.userData.isParticleGroup=true;return group;
}}
...
function placeLamps(){{
  ...
  for(let c=-1;c<=5;c++) for(let r=-1;r<=DIST_ORDER.length;r++){{
    const lx=-((5-1)*BLK_X)/2+c*BLK_X-BLK_X/2,lz=startZ+r*BLK_Z-BLK_Z/2;
...
function buildCar(){{
  ...
  const bM=new THREE.MeshLambertMaterial({{color:0x1a3060}});const dM=new THREE.MeshLambertMaterial({{color:0x080c14}});
...
function gameLoop(){{
  ...
  if(!modalOpen){{
    ...
    if(U)carSpeed=Math.min(carSpeed+ACCEL,MAX_SPD);else if(D)carSpeed=Math.max(carSpeed-ACCEL,-MAX_SPD*.5);else carSpeed*=FRIC;
    if(Math.abs(carSpeed)>.004){{const dir=carSpeed>0?1:-1;if(L)carAngle+=TURN*dir;if(R)carAngle-=TURN*dir;}}
    ...
    if(engineOsc&&audioCtx){{engineOsc.frequency.value=55+Math.abs(carSpeed)/MAX_SPD*80;engineGain.gain.value=Math.abs(carSpeed)>.01?.04:0;}}
  }}
  ...
  if(nearEntry&&nearEntry!==lastNearEntry)playChime();lastNearEntry=nearEntry;
  buildings.forEach(b=>{{
    ...
    b.particleGroup.children.forEach(mesh=>{{
      ...
      ud.baseAngle+=ud.speed;mesh.position.x=Math.cos(ud.baseAngle)*ud.radius;mesh.position.z=Math.sin(ud.baseAngle)*ud.radius;
      mesh.position.y=ud.yBase+Math.sin(frame*ud.ySpeed+ud.phase)*ud.yAmp;mesh.rotation.x+=0.02;mesh.rotation.y+=0.015;
...
function drawMinimap(){{
  ...
  mmCtx.fillStyle='#ffffff';mmCtx.beginPath();mmCtx.moveTo(0,-5.5);mmCtx.lineTo(3.2,4);mmCtx.lineTo(0,2);mmCtx.lineTo(-3.2,4);mmCtx.closePath();mmCtx.fill();mmCtx.restore();
...
</html>""".replace("{RESUME_B64}", resume_b64)

components.html(HTML_TEMPLATE, height=760, scrolling=False)
